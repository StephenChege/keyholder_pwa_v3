...
export default function ControlPanel({
  ...
  // When Proximity Response is ON, automatically update LED/Buzzer based on proximity
  useEffect(() => {
    if (proximityResponseEnabled && connectedDevice && ledOn && buzzerOn) {
      const outputValue = getProximityOutputValue();
      
      // Send the proximity-based value to both LED and Buzzer
      sendLedBrightness(outputValue);
      sendBuzzerVolume(outputValue);
      
      // Update display sliders to show current proximity-driven values
      setLedBrightness(Math.round((outputValue / 255) * 100));
      setBuzzerVolume(Math.round((outputValue / 255) * 100));
